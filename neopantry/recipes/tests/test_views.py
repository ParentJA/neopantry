...
class RecipeSearchViewTest(APITestCase):
    ...
    def test_saving_course_updates_course_search_words(self):
        recipe = RecipeFactory()
        recipe = Recipe.objects.get(id=recipe.id)
        self.assertGreater(RecipeSearchWord.objects.filter(recipe=recipe).count(), 0)

    def test_deleting_course_deletes_course_search_words(self):
        recipe = RecipeFactory()
        recipe.delete()
        self.assertEqual(RecipeSearchWord.objects.filter(recipe=recipe).count(), 0)

    def test_user_can_search_recipes(self):
        # Given.
        recipe1 = RecipeFactory(name='Chicken Pot Pie')
        recipe2 = RecipeFactory(name='Apple Pie')
        RecipeFactory.create_batch(3, name='Test', description='This is a test.')
    ...
    def test_search_uses_trigram_similarity_on_search_words(self):
        # Given.
        recipe1 = RecipeFactory(name='Chicken Pot Pie')
        recipe2 = RecipeFactory(name='Apple Pie')

        # When.
        # The misspelled word "chikcen" is similar enough to the word "chicken", so a match will be found for recipe1.
        # Nothing in recipe2 will match the query, so it will not be found.
        response = self.client.get(path=reverse('recipes:recipe-search'), data={'page': 1, 'query': 'chikcen'})
